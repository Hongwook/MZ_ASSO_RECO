...
inventory = pd.merge(inventory, prod_bar, on='barcode', how='left').groupby('product_id')['amount'].sum().reset_index()
...
result = result[['antecedents', 'consequents', 'lift', 'amount', 'ante_prod_nm', 'cons_prod_nm']].sort_values(
    by=['antecedents', 'lift'], ascending=[True, False]).head(10)
